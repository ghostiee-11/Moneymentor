...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- UPDATED CHART COLORS FOR COMIC THEME ---
    const CHART_COLORS = [
        '#4299e1', // Comic Accent Blue
        '#805ad5', // Comic Accent Purple
        '#f6e05e', // Comic Accent Yellow
        '#ed64a6', // Comic Accent Pink
        '#68d391', // Comic Accent Green
        '#f87171', // Light Red (Error color mapped)
        '#fbbf24', // Amber (Warning color mapped)
        '#34d399', // Teal (Success color mapped)
    ];
        ...
        if (allocationList) {
            ...
            if (data.allocation && allocationKeys.length > 0) {
                Object.entries(data.allocation).forEach(([asset, percentage]) => { /* ... (list item creation same as before) ... */
                    ...
                    // Use the assigned color for the border-left style
                    listItem.style.borderLeft = `4px solid ${assetColors[asset] || '#ccc'}`; // Use 4px solid border
                    ...
                    // No separate color indicator needed in list if border is colored
                    listItem.appendChild(assetSpan); listItem.appendChild(percentageSpan);
        ...
        if (periodData.required_total_investment !== null && periodData.required_total_investment > 0 && Object.keys(validBreakdown).length > 0) {
            ...
            Object.entries(validBreakdown).forEach(([asset, amount]) => {
                ...
                const assetSpan = document.createElement('span');
                 // Use the assigned color for the list item text/bullet? Or keep simple.
                 // Keeping simple for now, color indicators are best in allocation list.
                assetSpan.textContent = escapeHtml(asset);
                ...
                chartColors.push(assetColors[asset] || '#cccccc'); // Use the same colors as main chart
            });
            ...
            message.style.color = 'var(--comic-text-secondary)'; // Use comic muted text color
            message.style.fontSize = '0.9em';
            message.style.fontStyle = 'italic'; // Match other info text
    ...
    function renderAllocationChart(allocationData, assetColors) {
        ...
        // Data is percentages for the main chart
        const data = Object.values(allocationData).map(val => (val * 100).toFixed(1));
            ...
            allocationChartInstance = new Chart(ctx, {
                ...
                data: {
                    ...
                    datasets: [{
                        ...
                        // Border color from comic theme variables
                        borderColor: 'var(--comic-card-bg)', // Use the white card background as border
                        borderWidth: 4, // Thicker border
                        hoverOffset: 6 // Slightly larger hover offset
                    }]
                },
                options: {
                    responsive: true,
                        ...
                        tooltip: {
                            callbacks: {
                                label: function(context) {
                                    return `${context.label || ''}: ${context.parsed?.toFixed(1) || 0}%`;
                                }
                            },
                             // --- Tooltip Styling (using comic theme variables) ---
                             backgroundColor: 'var(--comic-card-bg)', // White background
                             titleColor: 'var(--comic-text-dark)', // Dark title
                             bodyColor: 'var(--comic-text-dark)', // Dark body text
                             borderColor: 'var(--comic-border-dark)', // Dark border
                             borderWidth: 2, // Thinner border for tooltip
                             cornerRadius: 6, // Slightly rounded corners
                             displayColors: true, // Show color square
                             boxPadding: 4 // Padding around color square
                        }
                    },
                    cutout: '65%'
                }
            ...
            breakdownChartInstances[periodName] = new Chart(ctx, {
                ...
                data: {
                    ...
                    datasets: [{
                        ...
                        backgroundColor: backgroundColors,
                        // Border color from comic theme variables
                        borderColor: 'var(--comic-bg-primary)', // Use the light background as border
                        borderWidth: 2,
                        hoverOffset: 4
                    }]
                },
    ...
    function handleLogInvestment() {
        ...
        logStatus.className = 'log-status-message'; // Reset classes first
        setTimeout(() => {
            logStatus.textContent = 'Monthly investment logged (Simulated).';
            logStatus.classList.add('success'); // Add success class for color
        }, 1000);
    }
